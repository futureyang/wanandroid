...
class LatestViewModel @ViewModelInject constructor(
    val lastestRepository: LatestRepository,
    ...
    fun refresh() {
        ...
        launch(
            block = {
                val pagination = lastestRepository.getArticleList(INITIAL_PAGE)
    ...
    fun loadMore() {
        launch({
            val articleData = lastestRepository.getArticleList(page)
